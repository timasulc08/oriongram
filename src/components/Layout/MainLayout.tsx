...
export const MainLayout: React.FC = () => {
  ...
  const myProfileId = useStore((s) => s.myProfile?.id) || null;
  const uid = myProfileId || auth.currentUser?.uid || null;

  const isMobile = useMediaQuery('(max-width: 768px)');

  // Чаты + звонки стартуют только когда есть uid
  useEffect(() => {
    if (!uid) return;

    setLoading(true);

    const unsubChats = subscribeChats(uid, (chats) => {
      setChats(chats);
      setLoading(false);
    });

    // PeerJS (можешь временно закомментировать, если peerjs.com у тебя блокируется)
    initPeer()
      .then((peerId) => console.log('[App] PeerJS ready:', peerId))
      .catch((e) => console.warn('[App] PeerJS init failed:', e));

    // Входящие звонки через Firestore
    const unsubCalls = listenForCalls((data: any) => {
      if (!data?.callerName || !data?.callerPeerId) return;
            ...
            <AnimatePresence mode="wait">
              {currentChatId ? (
                ...
                >
                  ...
                  <Typography variant="h5" sx={{ color: t.onSurface, fontWeight: 600 }}>
                    OrionGram
                  </Typography>
